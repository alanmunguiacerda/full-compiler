...
const string opelog[] = {"not", "or", "and" };
...
void insTabSim(string nmb, string cla, string tip,
               string d1, string d2) {
    tabSim[simn].nom = nmb;
...
void udimen() {
    int idx = buscaTabSim( iden );
    simbolos simbo = TabSim[iden];
    if( simbo.dimen1 == "0")
        cout << "[" << reng << "]" << "[" << colo << "] "
             << " Error de Semantica el identificador " + iden +
             " No esta dimensionado" << endl;
    /*
    if( idx >=0 && tabSim[idx].dimen1 == "0")
        cout << "[" << reng << "]" << "[" << colo << "] "
             << " Error de Semantica el identificador " + iden +
             " No esta dimensionado" << endl;
    */
    ...
    if( tp != "E")
       cout << "[" << reng << "]" << "[" << colo << "] "
            << " Error de Semantica se esperaba tipo Entero "
            << " en dimensiones llego " + tp << endl;
...
void asigna() {
    if( lex == "[") udimen();
        ...
        simbolos sim = TabSim[iden];
        if( sim.dimen1 != "0" )
             cout << "[" << reng << "]" << "[" << colo << "] "
                      << iden << " debe estar dimensionado "<< endl;
    ...
    do {
        ...
        if( op == "*" || op == "/" || op == "%") {
            ...
            if( (i = buscaTipo(tp) ) >= 0)
                ...
                ptipos.push("I");
                cout << "[" << reng << "]" << "[" << colo << "] "
                         << " Conflicto en tipos en la operacion " << op <<" <"
                         << tp << ">" << endl;
            }
        }
...
void ife() {
    ...
    expr();
     ...
     if( tp != "L") {
         cout << "[" << reng << "]" << "[" << colo << "] "
              << " Se esperaba tipo Logica en expresion de if y llego " + tp << endl;
     }
    ...
    if(lex == "else") {
        ...
        lex = lexico();
        block();
        insTabSim(Ey, "I", "I", convertInt(codn), "0" );
    }
    else {
        char s[12];
        insTabSim(Ex, "I", "I", convertInt(codn), "0" );
...
void fore() {
    ...
    if (lex != "{") {
       dir2 = codn;
       estatuto();
       genCod("JMP", "0", convertInt(dir) );

    }
    insTabSim(Ey, "I", "I", convertInt(codn), "0" );
    block();
    genCod("JMP", "0", convertInt(dir2) );
    insTabSim(Ex, "I", "I", convertInt(codn), "0" );
...
void funcs() {
    ...
    if (banMain)  genCod("OPR", "0", "0");
        ...
        cout << "[" << reng << "]" << "[" << colo << "] "
            << " la func main NO ha sido declarado" << endl;
    }
}
